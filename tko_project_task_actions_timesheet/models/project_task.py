...
class ProjectTask(models.Model):
    ...
    def write(self, vals):
        ...
        if (self.project_id.estimated_time > self.project_id.estimated_time_limit) and (
                    self.project_id.estimated_time_limit > 0.0):
...
class ProjectTaskActionsLine(models.Model):
    ...
    state = fields.Selection([('n', u'New'), ('i', u'In Progress'), ('d', u'Done'), ('c', u'Cancelled')], default='i',
                             required=True,
                             string='State')
...
class Timesheet_time(models.Model):
    ...
    def add_time(self):
        if self._context.get('active_model') == 'project.task.action.line':
            action_line = self.env['project.task.action.line'].search(
